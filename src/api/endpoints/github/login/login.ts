...
import type {
  ILoginViaGithubParams,
  ILoginViaGithubResponse,
  IRefreshRequestParams
} from './types';
...
const login = Object.freeze({
  ...
  refresh: ({
    ...
    client_id,
    client_secret
  }: IRefreshRequestParams): Promise<Response<IGithubApiResponse<ILoginViaGithubResponse>>> => {
    ...
    return process.env.NODE_ENV === 'development'
      ...
      : githubAPI.post('/login/oauth/access_token', null, {
          headers: {
            Accept: 'application/json'
          }
        });
